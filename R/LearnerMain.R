...
learner.get_model_hyperparameters <- function(data_obj, learner, names_only=FALSE, outcome_type=NULL){
  ...
  # Extract parameters
  param   <- learner.main(data_obj=data_obj, learner=learner, purpose="parameters", outcome_type=outcome_type)
...
learner.check_outcome_type <- function(learner, outcome_type){
  ...
  # Initialise variable
  type_is_valid <- learner.main(learner=learner, purpose="outcome", outcome_type=outcome_type)
  
  # Check if type_is_valid has been set. If not, the learner has not been specified (correctly) and an error should be given
  # If it has been set, but is FALSE, the current learner is incompatible with the outcome type provided.
...
learner.check_model_hyperparameters <- function(learner, user_param, outcome_type){
  ...
  defined_param <- learner.get_model_hyperparameters(data_obj=NULL, learner=learner, outcome_type=outcome_type, names_only=TRUE)
...
learner.check_model_prediction_type <- function(learner, outcome_type){
  ...
  return(learner.main(learner=learner, outcome_type=outcome_type, purpose="prediction_type"))
