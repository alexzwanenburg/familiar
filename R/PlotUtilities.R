...
plotting.parse_color_by <- function(x, color_by) {
  if (is.null(color_by)) {
    ...
    # Expand grid
    y <- data.table::data.table(expand.grid(lapply(
      color_by,
    ...
    # Determine levels
    y$color_by_col <- factor(
      x = y$color_by_col, 
      levels = y$color_by_col)
...
plotting.parse_linetype_by <- function(x, linetype_by) {
  if (is.null(linetype_by)) {
    ...
    # Determine levels
    y$linetype_by_col <- factor(
      x = y$linetype_by_col,
      levels = y$linetype_by_col)
