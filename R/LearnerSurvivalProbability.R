learner.survival.baseline_survival <- function(data_obj){
  ...
  survival_data <- unique(data_obj@data[, c("subject_id", "cohort_id", "outcome_time", "outcome_event"), with=FALSE])
...
learner.survival_probability.relative_risk <- function(object, data_obj, time_max){

  # Suppress NOTES due to non-standard evaluation in data.table
  survival <- NULL
  
  # Predict the data
  if(class(data_obj) == "dataObject"){
    prediction_table <- predict(object=object, newdata=data_obj, allow_recalibration=TRUE, time_max=time_max)
  } else if(class(data_obj) == "list"){
    prediction_table <- data_obj$predictions
  }

  if(nrow(prediction_table) <= 1){
    return(NULL)
    
  } else if(!any_predictions_valid(prediction_table=prediction_table, outcome_type=object@outcome_type)){
    return(NULL)
  }
  ...
  pred_risks <- prediction_table$outcome_pred
  ...
  baseline_surv <- stats::approx(x=object@calibration_info$time,
                                 ...
                                 xout=time_max, method="linear", rule=2)$y
  
  # Create a n_sample x n_times matrix
  survival_probability <- sapply(pred_risks, function(rr, s0) (s0^rr), s0=baseline_surv)
  
  # Add predicted probabilities to a table.
  probability_table <- data.table::data.table("subject_id"=prediction_table$subject_id,
                                              "survival"=survival_probability,
                                              "time"=prediction_table$outcome_time,
                                              "event"=prediction_table$outcome_event)
