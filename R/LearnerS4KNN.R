...
setMethod("get_default_hyperparameters", signature(object="familiarKNN"),
          function(object, data=NULL){
            ...
            if(object@learner == "k_nearest_neighbours"){
              gamma_default <- -5
              gamma_range <- c(-5, -5)
              
            } else if(object@learner == "k_nearest_neighbours_radial") {
              gamma_default <- c(-5, -3, -1, 0, 1, 3, 5)
              gamma_range <- c(-5, 5)
              
            } else {
              ..error_reached_unreachable_code(paste0("get_default_hyperparameters,familiarKNN: encountered unknown learner: ", object@learner))
            }
