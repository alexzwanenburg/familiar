...
metric.check_outcome_type <- function(metric, outcome_type){

  # Initialise variable
  type_is_valid <- metric.main(metric=metric, purpose="outcome", outcome_type=outcome_type)

  # Check if type_is_valid has been set. If not, the metric was not specified (correctly) and an error should be given
  # If it has been set, but is FALSE, the current metric is incompatible with the outcome type provided.
  if(is.null(type_is_valid)){
    ...
    stop(paste0(metric, " metric is not available for ", outcome_type, " outcomes."))
  }
}
...
metric.get_metric_default_range <- function(metric){
  # Get default range of metric scores, e.g. for plotting metric values.
  metric_score_range <- metric.main(metric=metric, purpose="default_range")
  
  return(metric_score_range)
}



metric.is_higher_score_better <- function(metric){

  higher_better <- metric.main(metric=metric, purpose="higher_score_better")
