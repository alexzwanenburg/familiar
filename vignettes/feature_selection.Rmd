...
| impurity^b^ | `random_forest_ranger_impurity`                         | × | × | × | × |   |
| **special methods**|
| no selection | `none`                                                 | × | × | × | × | × |
...
| splitting rule (`ranger` only) | `split_rule` | `gini`, `extratrees`, `variance`, `logrank`, `C`, `maxstat`| no | Default splitting rules are `gini` for `binomial` and `multinomial` outcomes and `maxstat` for `continuous`, `count` and `survival` outcomes.|
