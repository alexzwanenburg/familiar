...
# Performance metrics
When we train a model we usually want to know how good the model is. Model performance is assessed using different metrics that quantify how well a model discriminates cases, stratifies groups or predicts values. `familiar` implements many metrics that typically used to assess the performance of categorical, regression and survival models.
...
against a threshold. The reciever-operating curve is created by iterating over1
threshold values. The AUC of a model that predicts perfectly is $1.0$, whereas
...
classes. When dealing with two classes, e.g. *a* and *b*, one of the classes
is usually termed 'positive' and the other 'negative'. For example, let *b* be
the 'positive' class. Then we can define the following four categories:
...
Models that are not as good may have larger numbers of false positives and/or false negatives.
...
'positive' class, with the remainder grouped as 'negative'. Three options can be
used to obtain performance values for `multinomial` problems, with an
implementation similar to `scikit.learn`:

* `micro`: True positives, true negatives, false positives and false negatives
are computed for each class iteration, and then summed over all classes. The
score is subsequently calculated.
...
#### Balanced accuracy
Accuracy is known to be sensitive to imbalances in the classs distribution.
A balanced accuracy was therefore defined [@Brodersen2010-vb], which is the
...
and predicted classes [@Cohen1960-kc]. Cohen's kappa coefficient is invariant
to class permutations and no averaging is performed for Cohen's kappa. 
...
Youden's J statistic [@Youden1950-no] is the sum of recall and specificity
minus 1: $s_{youden} = TP / (TP + FN) + TN / (TN + FP) - 1$.

Youden's J statistic is not invariant to class permutatiosn and averaging is
performed for `multinomial` outcomes.
...
Mean squared log error is defined as $1/N \sum_i^N \log\left(y_i + 1\right) - \log(\hat{y}_i + 1\right)^2$. Note that this score only applies to observed and predicted values in the positive domain. It is not defined for negative values.
...
Here $\bar{y}$ denotes the mean value of $y$.
...
The root mean square log error is defined as $\sqrt{1/N \sum_i^N \log\left(y_i + 1\right) - \log(\hat{y}_i + 1\right)^2}$. Note that this score only applies to observed and predicted values in the positive domain. It is not defined for negative values.
...
The concordance index assesses ordering between observed and predicted values. Let $T$ be overserved times, $c$ the censoring status ($0$: no observed event; $1$: event observed) and $\hat{T}$ predicted times. Concordance between all pairs of values is determined as follows *reference missing*:
