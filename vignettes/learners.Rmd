...
| radial kernel^c^ | `svm_*_rbf`, `svm_*_radial`                        | × | × | × | × |   |
...
^c^ SVM type is indicated by `*`, and must be one of `c`, `c_bound`, `nu`, `epsilon` or `epsilon_bound`.
...
Boosting is a procedure which combines many weak learners to form a more powerful panel [@Schapire1990-vn; @Hastie2009-ed]. Extreme gradient boosting is a gradient boosting implementation that was very successful in several machine learning competitions. Learners are implemented using the `xgboost` package [@Chen2016-lo]. Three type are implemented: regression based boosting (`xgboost_lm`), regression-tree based boosting (`xgboost_tree`) and regression-tree based boosting with dropouts (`xgboost_dart`).
...
* Every second iteration, candidates are drawn randomly from both random configurations and random configurations in the local neighbourhood of the current best candidate, i.e. the acquisition function (e.g. expected improvement) is ignored. Originally, the algorithm only selected candidates using the acquisition function. This change forces the algorithm to more fully explore the hyperparameter space.
* The metric used for computing the optimisation score is not the logarithm of computation time, and can be chosen from any of the available performance metrics.

In summary, the algorithm works as follows:

1. A set of bootstraps ($n$ equals `optimisation_bootstraps`) of the relevant development dataset is created.
    + Variable importance is computed for the in-bag (IB) data of each bootstrap to avoid optimistic biases, unless `optimisation_determine_vimp` is `FALSE`.
2. An initial set of at most 100 unique hyperparameter configurations is randomly drawn from the hyperparameter space. Models are build using each set of hyperparameters and optimisation scores are computed.
3. A random-forest is trained using the hyperparameters as input variables and the corresponding optimisation scores as response variables. This model maps the hyperparameter space to optimisation scores.
4. Sequential model-based optimisation starts:
    + The configuration with the best optimisation score is selected.
    + New configurations are created both randomly and from the neighbourhood of the best configuration. Alternatingly, a number of the most promising candidates (based on an acquisition function) or a set of randomly drawn configurations are used as candidate configurations.
    + The candidate and incumbent configurations go through a number of intensify rounds. This is a run-off to determine which configuration produces the best optimisation score.
    + The random forest is updated with the new data generated during the run-off.
5. The above is repeated until the allowed number of iterations is exceeded, the algorithm has converged to a stable solution, or the hyperparameter space and the bootstraps have been exhausted.
...
The optimisation score $s$ is subsequently computed using one of the following functions:
...
all available optimisation scores.
...
and  Note that this acquisition function is
typically prone to convergence to local minima, although in our algorithm this
may be mitigated by the alternating random search strategy.
...
$$\alpha(\mathbf{x})=\mu(\mathbf{x}) + \beta \sigma(\mathbf{x})$$
...
Note that we here deviate from the original definition by substituting the
original $t$ by $t+1$, and not constraining the posterior distribution $\nu$ to
a known function. In the original, $n$ defines the number of rounds, with $t$
the current round. In our algorithm, this is interpreted as $n$ being the total
number of bootstraps, and $t$ the number of bootstraps already performed for the
selected hyperparameter set $\mathbf{x}$. Hence our implementation is actually
the same, aside from the lack of constraint on the posterior distribution.

Based on the simulations and recommendations in Kaufmann et al. we choose
parameter $c=0$.
