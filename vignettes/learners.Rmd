...
| linear kernel^c^ | `svm_*_linear`               | × | × | × | × |   |
| polynomial kernel^c^ | `svm_*_polynomial`       | × | × | × | × |   |
| radial kernel^c^ | `svm_*_radial`               | × | × | × | × |   |
| sigmoid kernel^c^ | `svm_*_sigmoid`             | × | × | × | × |   |
...
^c^ SVM type is indicated by `*`, and must be one of `c`, `nu`, or `epsilon`.
...
|:--------------|:--------|:----------:|:----------:|:-------------|
| signature size | `sign_size` | $\mathbb{Z} \in \left[1,n\right]$ | yes | -- |
| SVM kernel | `kernel` | `linear`, `polynomial`, `radial`, `sigmoid` | no | Default is `radial`, unless specified as part of the learner name. |
