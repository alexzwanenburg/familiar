...
* The intensify step, where challenger configurations are compared with the best
known configuration, is done in parallel, not sequentially.

* During the intensify step the likelihood that a promising challenger
configuration is at least as good as the best known configuration is assessed
using a paired, non-parametric Wilcoxon signed-rank test. If it is highly
unlikely to be better (default: $p \leq 0.05$), the candidate is eliminated from
...
using the acquisition function. This change forces the algorithm to more fully
