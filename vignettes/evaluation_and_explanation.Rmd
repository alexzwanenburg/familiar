...
An ROC-curve is created by sorting instances by the predicted probability of the positive class ascendingly, and computing the number of true positive, true negative, false positive, false negative cases observed at each instance. These are then used to compute sensitivity and specificity.

The above plot shows the average of 400 single curves, with 95% confidence intervals. To achieve this, familiar interrogates each curve over the complete range of potential specificity values $[0.000, 1.000]$ with step sizes of $0.005$. This allows for assessing the distribution of sensitivity at fixed specificity values. However, since only the maximum sensitivity value at each specificity is kept, the curve can no longer be represented as a step function.
...
Under ideal circumstances, the curve would lie in the top-right part of the plot. A random classifier would yield a curve that is mostly horizontal and position at the fraction of positive instances. This would equal a precision of 45% for the current dataset.
